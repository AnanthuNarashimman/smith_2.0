...
async function forceGoalChange(client, containerTag, currentGoalId, newGoal, contestedAction) {
  ...
  // approving a flagged action anyway — the developer chose to make the objection go
  // away rather than stick to what they originally said, so it should count against
  // the Consistency Score the same way an approved-then-executed flag does.
...
async function runArgueChat({ client, containerTag, config, tui, openingLine, currentGoalId, contestedAction }) {
  ...
  while (true) {
    ...
    if (pendingDraft && CONFIRM_PATTERN.test(userText)) {
      ...
      try {
        const goal = await forceGoalChange(client, containerTag, currentGoalId, pendingDraft, contestedAction);
