...
function ensureHooksConfigured(port = 6789) {
  ...
  // PostToolUseFailure is a separate event from PostToolUse — Claude Code only fires
  // PostToolUse when the tool call succeeds, so a flagged-then-approved action whose
  // command errors out (e.g. a package manager failing) would otherwise leave no trace
